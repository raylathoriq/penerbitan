        ...
        <x-card>
            ...
            <form action="{{ route('author.naskah.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-6">
                    ...
                    {{-- PENULIS --}}
                    <div x-data="{ coAuthors: [] }" class="rounded-xl border border-slate-200 p-5 bg-white">
                        ...
                        <div class="flex justify-between items-center mb-3">
                            ...
                            <button type="button" @click="coAuthors.push({ name:'', email:'', affiliation:'' })"
                        ...
                        <template x-for="(author, index) in coAuthors" :key="index">
                            <div class="relative p-4 mb-3 bg-slate-50 border border-slate-100 rounded-lg">
                                ...
                                <button type="button" @click="coAuthors.splice(index, 1)"
                                    class="absolute top-3 right-3 text-rose-500">
                                    ✕
                                </button>

                                <div class="space-y-2">
                                    <input type="text" x-model="author.name" placeholder="Nama Lengkap"
                                        class="w-full rounded-lg border-slate-200 p-2 text-sm">

                                    <div class="grid md:grid-cols-2 gap-2">
                                        <input type="email" x-model="author.email" placeholder="Email"
                                            class="w-full rounded-lg border-slate-200 p-2 text-sm">

                                        <input type="text" x-model="author.affiliation" placeholder="Institusi"
                                            class="w-full rounded-lg border-slate-200 p-2 text-sm">
                                    </div>
                                </div>
                            </div>
                    ...
                    <div>
                        ...
                        <div class="border-2 border-dashed border-slate-200 rounded-xl p-6 text-center bg-slate-50">
                            <input type="file" name="document" accept=".pdf,.doc,.docx"
                                class="text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100"
                                required>
                        </div>
                        @error('document')
                    ...
                    <div class="flex items-start gap-3 bg-slate-50 p-4 rounded-lg border">
                        <input type="checkbox" required class="mt-1 h-4 w-4 text-emerald-600">
