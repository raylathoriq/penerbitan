    ...
    <div class="hidden md:flex md:flex-shrink-0">
        <div class="flex flex-col w-64 border-r border-slate-200 text-slate-700 bg-white shadow-sm z-20">
            <div class="h-16 flex items-center px-6 border-b border-slate-100">
                <span class="text-xl font-bold tracking-tight text-slate-900">Reviewer Panel </span>
            </div>
            <div class="flex-1 flex flex-col overflow-y-auto p-4">
                <nav class="flex-1 space-y-1.5">
                    <a href="/reviewer/dashboard" class="{{ request()->is('reviewer/dashboard') ? 'bg-indigo-50 text-indigo-800' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }} group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors">
                        Dashboard
                    </a>
                    <a href="/reviewer/naskah" class="{{ request()->is('reviewer/naskah*') ? 'bg-indigo-50 text-indigo-800' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }} group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors">
                        Penugasan Naskah
                    </a>
                </nav>
            </div>
                ...
                <div class="ml-4 flex items-center md:ml-6">
                    <div class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" @click.away="open = false" 
                            ...
                            <a href="/reviewer/profil" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-50">Profil Saya</a>
                            <div class="border-t border-slate-100 my-1"></div>
            ...
            <div class="max-w-6xl mx-auto">
                ...
                @if(session('success'))
                    <div 
                        ...
                        class="mb-6 flex items-center justify-between p-4 rounded-xl bg-emerald-50 border border-emerald-200/60 text-emerald-800 shadow-sm"
                    >
                ...
                @if(session('error'))
                    <div 
                        ...
                        x-transition:leave-start="opacity-100 transform translate-y-0"
                        x-transition:leave-end="opacity-0 transform -translate-y-2"
                        class="mb-6 flex items-center justify-between p-4 rounded-xl bg-rose-50 border border-rose-200/60 text-rose-800 shadow-sm"
                    >
                        <div class="flex items-center gap-3">
                        ...
                        <button @click="show = false" class="text-rose-600 hover:text-rose-800 focus:outline-none">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                            </svg>
                        </button>
